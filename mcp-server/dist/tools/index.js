...
import { createTool } from "../utils/tool-mapper.js";
...
async function runGsdTool(command, args, cwd) {
...
const tools = [
    createTool("gsd_new_project", "Initialize a new project with GSD workflow", NewProjectSchema, async (args) => {
    ...
    createTool("gsd_plan_phase", "Research and create plans for a phase", PlanPhaseSchema, async (args) => {
    ...
    createTool("gsd_execute_phase", "Execute all plans in a phase", ExecutePhaseSchema, async (args) => {
    ...
    createTool("gsd_verify_work", "Verify phase completion with user acceptance testing", VerifyWorkSchema, async (args) => {
    ...
    createTool("gsd_discuss_phase", "Discuss phase implementation details before planning", DiscussPhaseSchema, async (args) => {
    ...
    createTool("gsd_progress", "Show current project progress and status", ProgressSchema, async (args) => {
    ...
    createTool("gsd_quick", "Execute a quick ad-hoc task", QuickSchema, async (args) => {
    ...
    createTool("gsd_map_codebase", "Analyze existing codebase", MapCodebaseSchema, async (args) => {
    ...
    createTool("gsd_new_milestone", "Start a new milestone", NewMilestoneSchema, async (args) => {
    ...
    createTool("gsd_complete_milestone", "Complete current milestone", CompleteMilestoneSchema, async (args) => {
    ...
    createTool("gsd_read_state", "Read project STATE.md file", ReadStateSchema, async (args) => {
        const projectPath = resolveProjectPath(args.cwd);
        const statePath = join(projectPath, ".planning", "STATE.md");
        try {
            const content = await readFile(statePath, "utf-8");
    ...
    createTool("gsd_run_cli", "Run any gsd-tools.js command directly", RunCliSchema, async (args) => {
    ...
    createTool("gsd_health", "Check GSD installation and project health", HealthSchema, async (args) => {
        const checks = [];
        const errors = [];
        const gsdToolsPath = getGsdToolsPath();
